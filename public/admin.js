const updateQuant = async (el, id) => {
  ...
  const main = async () => {
    ...
    books.forEach((book) => {
      ...
      submit.setAttribute('onClick', `updateQuant(this, ${id})`);
...
// async function main (){
//     let root = document.querySelector('#root');
//     let res = await fetch ('http://localhost:3000/listBooks')
//     let books = await res.json();
//     console.log(books);
// }

    // books.forEach(({description, id, imageURL, quantity, title, year})=> {
